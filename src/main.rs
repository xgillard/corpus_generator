...
/// 
/// http:://{server}/word/<secret>/<corpus_id>/<username>/<nth_random_word>
///
/// Where:
/// * `{server}` stands for the ip address/dns name of the machine where
///   this service is deployed.
...
///
#[rocket::get("/<secret>/<corpus_id>/<uname>/<nth_random_word>")]
async fn random_word_from_static_corpus(
    ...
    let fname = fname_from_corpus_id(corpus_id);
    let data  = pick_random_word(secret, &fname, uname, nth_random_word).await?;
    Ok(data)
}
